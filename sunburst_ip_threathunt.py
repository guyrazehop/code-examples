...
parser.add_argument('-v', '--verbose', dest='verbose', help='Control output', action='store_true')

parser.add_argument('--device-list-only', dest='deviceonly', help='Generate list of devices only', action='store_true')
...
parser.add_argument('--output-prefix', dest='prefix', help='Prefix for output files', default=None)
...
if args.devicesfile is None:
    ...
    with open(f'./{args.prefix+"_" if args.prefix else ""}device_ids.csv', 'w') as f:
    ...
    print(f'Found {len(device_details)} devices. Writing them to ./{args.prefix+"_" if args.prefix else ""}device_ids.csv')
    if args.deviceonly:
        exit()
else:
    if args.deviceonly:
        print(f'Devices file provided with --device-list-only. Please use one or the other. Exiting...')
        exit()
...
with open(f'./{args.prefix + "_" if args.prefix else ""}extrahop_sunburst_ip_hits.csv', 'w', buffering=1) as f:
